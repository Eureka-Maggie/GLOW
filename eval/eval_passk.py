...
def parse_args():
    ...
    parser.add_argument('--save_name', type=str, default="", help="model dir")
    ...
    parser.add_argument('--vllm_memory', type=float, default=0.8)
...
def infer(args):
    ...
    # for a_data_name in data_name_list:
    #     args.data_name=a_data_name
    ...
    result_file = f'{args.output_dir}/{model_name}/{args.data_name}/result.txt'
    
    if os.path.exists(out_file):
        print(f"Completely same name file({out_file}) exist, skip generation and save file, directly to metric/correct check")
        with open(out_file, 'r', encoding='utf-8') as f:
            file_outputs = [json.loads(line.strip()) for line in f]
        print('load done')
        save_flag=False
    else:
        ...
        for example in tqdm(examples, total=len(examples)):
            ...
            if args.surround_with_messages:
                if args.prompt_type == 'qwen-instruct-template' or args.prompt_type == 'qwen-base-template':
                    messages = [
        ...
        llm = LLM(model=model_name_or_path, 
                ...
                gpu_memory_utilization=args.vllm_memory #0.95,
                )
        ...
        for cur_generation_epoch in range(generation_epoch):
            ...
            for i in range(len(examples)):
                ...
                generated_length = [len(completions[i].outputs[j].token_ids) for j in range(len(completions[i].outputs))]
    ...
    for i in tqdm(range(len(examples)), "check correct..."):
        ...
        if ('gold_answer' in file_outputs[i]) and (file_outputs[i]['gold_answer']!=gt_ans):
            raise
        elif ('gold_answer' in file_outputs[i]) and (file_outputs[i]['gold_answer']==gt_ans):
            generated_answers = file_outputs[i]['generated_answers'] 
            gt_ans = file_outputs[i]['gold_answer']
            is_correct = file_outputs[i]['is_correct']
            if is_correct:
                correct_cnt += 1
    ...
    with open(result_file, "a") as f:
        f.write(f"---------------------------\n")
        f.write(f"num_sample={args.n_sampling}, k={args.k}, temp={args.temperature} ,topp={args.top_p}, seed={args.seed}\n")
        f.write(f"Avg Length: {sum(length_list_f) / len(length_list_f):.4f}\n")
        ...
        if pass_at_k_list:
            ...
            f.write(f"Pass@{k}: {sum(pass_at_k_list)}/{len(pass_at_k_list)} = {average_pass_at_k:.4f}\n")
        else:
