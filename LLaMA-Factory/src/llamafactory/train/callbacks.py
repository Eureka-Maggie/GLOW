...
class LogCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, **kwargs):
        ...
        # 1) 本次事件自带的日志（HF 会传进来）
        last_event = kwargs.get("logs", {}) or {}

        # 2) 找“最近一条包含动态指标”的记录；优先识别我们打的标记 _dyn_metrics
        dyn = None
        for rec in reversed(state.log_history):
            if isinstance(rec, dict) and (rec.get("_dyn_metrics") or "avg_loss_unweighted" in rec):
                dyn = rec
                break
        ...
        logs = dict(
            ...
            loss=last_event.get("loss"),
            eval_loss=last_event.get("eval_loss"),
            predict_loss=last_event.get("predict_loss"),
            reward=last_event.get("reward"),
            accuracy=last_event.get("rewards/accuracies"),
            lr=last_event.get("learning_rate"),
            epoch=last_event.get("epoch"),
        ...
        if dyn is not None:
            for k in (
                "avg_loss_unweighted", "std_weight", "avg_weight",
                "weight_min", "weight_max",
                "avg_delta_l", "std_delta_l",
                "percent_harder", "percent_easier",
                "weighted_loss",
            ):
                if k in dyn:
                    logs[k] = dyn[k]
        ...
        if self.webui_mode and all(key in logs for key in ("loss", "lr", "epoch")):
            ...
            for extra_key in ("reward", "accuracy", "throughput"):
                if logs.get(extra_key) is not None:
                    log_str += f", '{extra_key}': {logs[extra_key]:.2f}"
            # 顺便打印动态权重摘要
            if all(x in logs for x in ("avg_weight", "std_weight")):
                log_str += f", 'avg_weight': {logs['avg_weight']:.3f}, 'std_weight': {logs['std_weight']:.3f}"
